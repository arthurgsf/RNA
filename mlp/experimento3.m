...
for k = 1:10
    ...
    for i = 1:EPOCHS
        ...
        model = MLP(INPUT_SHAPE, OUTPUT_SHAPE, HIDDEN_NEURONS + 2*k, ACTIVATION, FDFH, 0.01);
        model = Fit(model, X_train, y_train);
        model = Validate(model, X_valid, y_valid);
        MEAN_T = MEAN_T + sum(model.TRAIN_ERROR);
        MEAN_V = MEAN_V + sum(model.VALID_ERROR);
    end
    HIST_N = [HIST_N, HIDDEN_NEURONS + 2*k];

    MEAN_T = MEAN_T/EPOCHS;
    HIST_T = [HIST_T, MEAN_T];
    
    MEAN_V = MEAN_V/EPOCHS;
    HIST_V = [HIST_V, MEAN_V];
end
HIST_V = cell2mat(HIST_V);
HIST_T = cell2mat(HIST_T);
HIST_N = cell2mat(HIST_N);

figure
hold on
plot(HIST_N, HIST_T, 'r', HIST_N, HIST_V, 'b');
legend('treino', 'validação');
title("Neurônios x Error");
